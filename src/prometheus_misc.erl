...
behaviour_modules(Behaviour) ->
    ...
    Modules = get_all_modules(Applications),
    Targets = lists:usort(Modules),
    extract_behaviour_from_modules(Targets, Behaviour).

-spec get_all_modules([{atom(), string(), string()}]) -> [module()].
get_all_modules(Applications) ->
    AllModules = [get_modules_for_app(App) || App <- Applications],
    lists:flatten(AllModules).
