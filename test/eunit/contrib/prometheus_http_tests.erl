...
status_class_test() ->
    ?assertError(
        {invalid_value, "qwe", "status code must be a positive integer"},
        prometheus_http:status_class("qwe")
    ),
    ?assertError(
        {invalid_value, 1.2, "status code must be a positive integer"},
        prometheus_http:status_class(1.2)
    ),
    ?assertError(
        {invalid_value, -10, "status code must be a positive integer"},
