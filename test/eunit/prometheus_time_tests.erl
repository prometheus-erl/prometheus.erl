...
from_native_test() ->
    ...
    ?assertEqual(2.5, prometheus_time:from_native(microseconds, 2.5 * NativeInUS)),
    ?assertEqual(2.6, prometheus_time:from_native(milliseconds, 2.6 * 1000 * NativeInUS)),
    ?assertEqual(2.23, prometheus_time:from_native(seconds, 2.23 * 1000000 * NativeInUS)),
    ?assertEqual(
        3.4,
        prometheus_time:from_native(
            minutes,
            3.4 * 60 * 1000000 * NativeInUS
        )
    ),
    ?assertEqual(
        0.4,
...
to_native_test() ->
    ...
    ?assertEqual(
        trunc(2.6 * 1000 * NativeInUS),
        prometheus_time:to_native(2.6, milliseconds)
    ),
    ?assertEqual(
        trunc(2.23 * 1000000 * NativeInUS),
        prometheus_time:to_native(2.23, seconds)
    ),
    ?assertEqual(
        trunc(3.4 * 60 * 1000000 * NativeInUS),
        prometheus_time:to_native(
            3.4,
            minutes
        )
    ),
    ?assertEqual(
        trunc(0.4 * 3600 * 1000000 * NativeInUS),
        prometheus_time:to_native(
            0.4,
            hours
        )
    ...
    ?assertEqual(
        trunc(0.1 * 86400 * 1000000 * NativeInUS),
        prometheus_time:to_native(
            0.1,
            days
        )
